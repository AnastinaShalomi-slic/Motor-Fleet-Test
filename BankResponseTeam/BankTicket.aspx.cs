...
public partial class BankResponseTeam_BankTicket : System.Web.UI.Page
{
    ...
    protected void sendEmailToSLICOfficer(string refNo, string bankCode, string req_type)
    {
        ...
        toEmails = String.Join(", ", emailIDForCC);
        //toEmails = String.Join(", ", emailID);
        //toEmails = bankEnmailId.Value.Trim().ToLower();
        emailSend.fireRequestDetails("BANKINQ", txt_body, toEmails, ccEmails, refNo.ToString().Trim().ToString(), Session["temp_bank"].ToString(), Session["temp_branch"].ToString(), Session["bancass_email"].ToString());
    ...
    protected void btfinished_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string Gen_t_id;
            result1 = exe_in.insert_ticket_details(txtRefNo.Text.ToString(), txtremark.Value, Session["bank_code"].ToString(), Session["branch_code"].ToString(), Session["userName_code"].ToString(), ddlStatus.SelectedValue.ToString().Trim(), "P", Session["temp_bank"].ToString(), Session["temp_branch"].ToString(), Session["bancass_email"].ToString().Trim(),out Gen_t_id);
    ...
    protected void btNewClick(object sender, EventArgs e)
    {
        ...
        tempRefNo.Value = txtRefNo.Text.ToString();
    }
