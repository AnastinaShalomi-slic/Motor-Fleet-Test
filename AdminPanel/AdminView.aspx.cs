...
public partial class AdminPanel_AdminView : System.Web.UI.Page
{
                ...
                if (Auth == "admin")
                {
                    ...
                    if (!string.IsNullOrEmpty(bank))
                    {
                        ...
                        if ((Session["userName_code"].ToString().Substring(4) == "admin"))
                        {
                            ...
                            //this.GetDetailsTotalCounts(txt_start_date.Text.Trim(), txt_to_date.Text.Trim(), ddl_status.SelectedValue.Trim().ToUpper().ToString());
    ...
    protected void GetDetails(string start_date, string end_date, string app_status)
    {
        ...
        try
        {
            ...
            details = orcle_trans.GetRows(this._sql.GetTotalPremiums(start_date, end_date, app_status, bank_val, branch_val, null, propTerm, propType), details);
    ...
    protected void GetDetailsTotalCounts(string start_date, string end_date, string app_status)
    {
        DataTable details = new DataTable();
        try
        {
            ...
            details = orcle_trans.GetRows(this._sql.GetTotalPremiumsCounts(start_date, end_date, app_status, bank_val, branch_val, null, propTerm, propType), details);
    ...
    protected void ImageButton8_Click(object sender, ImageClickEventArgs e)
    {
        ...
        else
        {
            ...
            var endc = new EncryptDecrypt();
            Response.Redirect("~/session_error/MessagePages.aspx?error=" + endc.Encrypt("ERROR_CLOSE".ToString()) + "&APP_MSG=" + endc.Encrypt(msg) + "&code=" + endc.Encrypt("8".ToString()));
    ...
    protected void btexcel_Click(object sender, EventArgs e)
    {
        ...
        if (Grid_Details.Rows.Count > 0)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=FirePolicyPremiums.xls");
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                string wz = "Fire Policy " + ddl_status.SelectedItem.ToString() + " Premiums Report (" + ddlTerm.SelectedItem.ToString() + "/" + ddlPropType.SelectedItem.ToString() + ")<br/>" +
    ...
    private void ExportGridToExcel()
    {
        ...
        propTerm = ddlTerm.SelectedValue.ToString().Trim();
        propType = ddlPropType.SelectedValue.ToString().Trim();
        dt = orcle_trans.GetRows(this._sql.GetTotalPremiums(txt_start_date.Text.Trim(), txt_to_date.Text.Trim(), ddl_status.SelectedValue.Trim().ToUpper().ToString(), bank_val, branch_val, null, propTerm, propType), dt);



        using (XLWorkbook wb = new XLWorkbook())
        {
            var ws = wb.Worksheets.Add("Fire_Policy_Premiums-" + DateTime.Now.ToString("yyyy-MM"));
            ...
            ws.Cell(2, 2).Value = "Fire Policy " + ddl_status.SelectedItem.ToString() + " Premiums Report";
            ...
            string[] ColumnHead = { "NO", "Bank", "Branch", "Count", "Entered Date", "Total Premium" };

            int[] ColumnSize = { 5, 25, 25, 25, 25, 25 };
            ...
            for (int rows = 0; rows < RowCount; rows++)
            {
                ...
                ws.Cell(9 + rows, 7).Value = dt.Rows[rows]["total"];
            ...
            Response.AddHeader("content-disposition", "attachment;filename=FirePolicyPremiums.xlsx");
