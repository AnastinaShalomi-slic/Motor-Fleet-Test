    ...
    public void print_policy(string policyNo)
    {
        ...
        if (Oracle_Trans.Trans_Sucess_State == true)
        {
            ...
            if (details.Rows.Count > 0)
            {
                ...
                //}
                if (details.Rows[0]["RNSTDT"] != DBNull.Value)
                {
                    RNSTDT = Convert.ToDateTime(details.Rows[0]["RNSTDT"]).ToString("yyyy/MM/dd");
                }
                else
                {
                    RNSTDT = ""; // or default value
                }
                ...
                if (details.Rows[0]["RNENDT"] != DBNull.Value)
                {
                    RNENDT = Convert.ToDateTime(details.Rows[0]["RNENDT"]).ToString("yyyy/MM/dd");
                }
                else
                {
                    RNENDT = ""; // or default value
                }


                // Inside your method
                RNNAM = details.Rows[0]["RNNAM"] != DBNull.Value ? details.Rows[0]["RNNAM"].ToString() : string.Empty;
                if (RNNAM.Contains("#160;"))
                {
                    RNNAM = RNNAM.Replace("#160;", " ");
                }
                if (RNNAM.Contains("&amp;"))
                {
                    RNNAM = RNNAM.Replace("&amp;", "");
                ...
                RNADD3 = details.Rows[0]["RNADD3"] != DBNull.Value ? details.Rows[0]["RNADD3"].ToString() : string.Empty;
                if (RNADD3.Contains("&nbsp;"))
                {
                    RNADD3 = RNADD3.Replace("&nbsp;", " ");
                }

                RNADD4 = details.Rows[0]["RNADD4"] != DBNull.Value ? details.Rows[0]["RNADD4"].ToString() : string.Empty;
                if (RNADD4.Contains("&nbsp;"))
                {
                    RNADD4 = RNADD4.Replace("&nbsp;", " ");
                }
        ...
        leftInner.AddCell(new Phrase(RNADD3, regularFont));
        leftInner.AddCell(new Phrase(RNADD4, regularFont));
        ...
        document.Add(new Paragraph("\n", bodyFont));
        Chunk specEndoP12 = new Chunk("1. The policy expires on the above specified date.", FontFactory.GetFont("Times New Roman", 8, Font.NORMAL, Color.BLACK));
