...
public partial class OdProtect_BackOffice_BusinessStatistics_ : System.Web.UI.Page
{
    ...
    protected void GetDetails()
    {
        ...
        try
        {
            resultset = oDP_Transaction.GetRows(_sql.GetOdpPerformance(txt_start_date.Text.Trim(), txt_to_date.Text.Trim(), ddl_status.SelectedValue.Trim(), int.Parse(ddl_bank.SelectedValue.Trim()), int.Parse(ddl_branch.SelectedValue.Trim())), resultset, "Overdraft Policy Statistics");
    ...
    protected void GetDetailsTotal()
    {
        ...
        try
        {
            ...
            {
              ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "Message('Error', 'Loading Overdraft Policies Statistics.[ORCL]', 'error');", true);

            }
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "Message('Exception', 'In Loading Overdraft Policy Statistics.', 'error');", true);
            ODP_LogExcp oDP_LogExcp = new ODP_LogExcp();
            string Error = "Exception :: Request  - (METH- GetDetailsTotal(..) -> - [USER : " + UserId.Value + "] - [BRANCH : " + brCode.Value + "]> ";
            oDP_LogExcp.WriteLog(Error + ex.Message + Environment.NewLine + System.Web.HttpContext.Current.Request.Url.AbsolutePath + Environment.NewLine);
        }
    }
