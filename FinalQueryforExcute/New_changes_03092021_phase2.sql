...
where n >= 2;
 ...
 QUOTATION.FIRE_DH_DEDUCTIBLES

select * from QUOTATION.FIRE_DH_BANCASSU_BORDEREAUX
select t.*, t.rowid from QUOTATION.BANCASU_RATE t

select * from QUOTATION.FIRE_DH_SCHEDULE_CALC tn
order by tn.created_on desc;
1	FFPD/2606		10500000.00	3050.00	3150.00	1312.50	26.29	500.00	0.00	617.50	8336.30	10/12/2021 2:32:02 PM	boc001	P 80.00

select * from QUOTATION.FIRE_DH_SCHEDULE_CALC_HISTORY tm
order by tm.created_on desc;
select * from QUOTATION.FIRE_DH_SCOPE_OF_COVERS@live
...
update QUOTATION.BANCASU_RATE set RENEWAL_FEE =100,BASIC_2=0.02,TERM='A'
where BASIC =0.04
and bank_code='7010'
...
select * from QUOTATION.FIRE_DH_SCHEDULE_CALC bn where bn. sc_policy_no='FPDL2119200206'--bn.sc_ref_no ='FFPD/2780'
...
CREATE TABLE QUOTATION.DISCOUNT_RATE
 (
  ...
  UPDATED_BY VARCHAR2(12)
    ...
    select * from QUOTATION.DISCOUNT_RATE tx
    where tx.bank_code =''
    and tx.no_of_years =
    
SELECT * FROM QUOTATION.FIRE_DH_SOLAR_RATE
------SOLAR
 CREATE TABLE QUOTATION.FIRE_DH_SOLAR_RATE
  ...
  truncate table QUOTATION.FIRE_DH_SOLAR_RATE
...
SELECT t.*, t.rowid from QUOTATION.FIRE_DH_CONTACT_EMAILS t
...
select EPF, BANK_CODE, BANK_NAME, OFFICER_NAME, DESIGNATION, CONTACT_NO, EMAIL
       ...
       where BANK_CODE = '7010'
        and trim(DESIGNATION) = trim('Sales Officer')
        and flag = 'A';
        

  select RATE
    FROM QUOTATION.FIRE_DH_SOLAR_RATE 
        where active = 'Y' 
        and BANK_CODE = '7010' 
        and MIN_VAL <= 2000000
        and MAX_VAL >= 2000000
...
--where t.sms_date_time is not null
order by t.record_sequence desc --remove @livefrom project
...
CREATE SEQUENCE QUOTATION.BANCASSU_BORDEREAUX_SEQ 
 ...
 CREATE TABLE QUOTATION.FIRE_DH_BANCASSU_BORDEREAUX
  ...
  select * from QUOTATION.FIRE_DH_BANCASSU_BORDEREAUX
  select * from QUOTATION.FIRE_DH_SCHEDULE_CALC bn where bn. sc_policy_no='FPDL2119200198'
  truncate table QUOTATION.FIRE_DH_BANCASSU_BORDEREAUX
...
--
select * from QUOTATION.FIRE_SCHEDULE_LONGTERM_TOTAL

DROP TABLE QUOTATION.FIRE_SCHEDULE_LONGTERM_TOTAL
CREATE TABLE QUOTATION.FIRE_SCHEDULE_LONGTERM_TOTAL
 (SC_REF_NO VARCHAR2(12 BYTE) NOT NULL ENABLE,
  ...
  CREATE TABLE QUOTATION.BANCASSU_UPMOTO_QUOTATIONS
   ...
   drop table QUOTATION.BANCASSU_UPMOTO_QUOTATIONS
   select t.*, t.rowid from QUOTATION.BANCASSU_UPMOTO_QUOTATIONS t
   truncate table QUOTATION.BANCASSU_UPMOTO_QUOTATIONS;
 /* select * from QUOTATION.NM_UPLOADED_QUOTATIONS*/
  --//////////////////
  ...
  CREATE SEQUENCE QUOTATION.bacassu_motoquo_seq
   ...
   select * from QUOTATION.BANCASSU_MOTOQUOREMARKS 
   where r_ref ='BOC962'
   and r_flag ='A'
   order by r_created_on desc;
   
   
  drop SEQUENCE QUOTATION.bacassu_motorespo_seq 
  ...
  CREATE SEQUENCE QUOTATION.bacassu_motorespo_seq
    ...
    CACHE 20;  
     ...
     select nm.r_no, nvl(nm.r_slic,'nn'), nvl(nm.r_bank,'bb') from QUOTATION.BANCASSU_MOTOQUOREMARKS nm
     where nm.r_ref = 'BOC962'
     and nm.r_flag ='A'
     order by nm.r_no desc;
    
    select * from POSTOFFICE.PO_OFFICER_DETAIL@live rt 
    where rt.emp_nic='100006121V' 

select distinct(br.bcode) as bcode
, trim(Upper(br.bbnam)) as bbnam
       from GENPAY.BNKBRN br  
        group by bcode, bbnam
        order by bbnam asc
        
        select REQ_ID from QUOTATION.BANK_REQ_ENTRY_DETAILS where REQ_ID
        
        SELECT EMAIL FROM QUOTATION.FIRE_DH_CONTACT_EMAILS
        select * from QUOTATION.DISCOUNT_RATE tx
        
        SELECT *
        --BANK, BASIC, RCC, TR, ADMIN_FEE, POLICY_FEE, Renewal_FEE,BASIC_2,TERM
        FROM QUOTATION.BANCASU_RATE
        
        
        
        UPDATE QUOTATION.BANCASSU_MOTOQUOREMARKS SET R_BANK = 'xxxx', R_UP_BY_BANK = 'b001', R_UP_ON_BANK = sysdate 
        WHERE R_REF = 'BOC962' AND R_NO = (select max(r_no) from QUOTATION.BANCASSU_MOTOQUOREMARKS where r_ref = 'BOC962' and r_flag = 'A')
        
        
        select nm.r_no, NVL(nm.r_slic, 'xx'), NVL(nm.r_bank, 'nn') 
        from QUOTATION.BANCASSU_MOTOQUOREMARKS nm where nm.r_ref = 'BOC962' and nm.r_flag = 'A' order by nm.r_no desc
        
SELECT * FROM QUOTATION.FIRE_POLICY_SEQ WHERE P_TYPE
FFPD/2779
...
SELECT * FROM dual;
  ...
  select t.*, t.rowid from QUOTATION.FIRE_RATE_LONGTERM_ALL_SUMINSU t

      select * 
        FROM QUOTATION.FIRE_RATE_LONGTERM_ALL_SUMINSU 
        where active = 'Y' 
        and BANK_CODE = '7010'
        and MIN_VAL <= 20000000
        and MAX_VAL >= 20000000
...
--solar deductibles
 select * from QUOTATION.FIRE_SOLAR_DEDUCTIBLES
DROP TABLE QUOTATION.FIRE_SOLAR_DEDUCTIBLES
 CREATE TABLE QUOTATION.FIRE_SOLAR_DEDUCTIBLES
  ...
  --truncate TABLE QUOTATION.FIRE_SOLAR_DEDUCTIBLES 
...
--common response team tables------------------------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>..
 ...
 --//////////////////
  ...
  CREATE SEQUENCE QUOTATION.bacassu_ticket_seq
   ...
   --alter table QUOTATION.BANCASSU_INQU_TICKETS add  T_BANK_EMAIL  VARCHAR2(100),
   select * from QUOTATION.BANCASSU_INQU_TICKETS
   
   ----//////////////////////// bank side uploadings table
...
select t.*, t.rowid from QUOTATION.BANCASSU_BANK_ITUPDOC t
...
select t.*, t.rowid from QUOTATION.BANCASSU_SLIC_ITUPDOC t
  ...
  CREATE SEQUENCE QUOTATION.bacassu_slic_ticket_seq 
    ...
    select T_REF from QUOTATION.BANCASSU_INQU_TICKETS where T_REF like '%T3%' AND ROWNUM <= 10 order by T_CREATED_ON desc
    
    select to_decimal SUBSTR('T104', 2) from dual;
    
    
    select t.*, t.rowid from QUOTATION.FIRE_DH_CONTACT_EMAILS t
        WHERE t.BANK_CODE = '7010' 
        AND t.FLAG = 'A'
        AND EMAIL_CC = 'N'  -- email cc N to ruween and pasidu in live
        
        
        
        select * from SMS.SMS_GATEWAY@live tb
        order by tb.record_sequence desc;
